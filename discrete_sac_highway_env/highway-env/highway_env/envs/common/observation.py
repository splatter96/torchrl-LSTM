...
class LidarObservation(ObservationType):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.grid_radar = np.ones((self.cells_radar, 1)) * float("inf")
        self.origin = None
    ...
    def observe(self):
        ...
        if self.enable_interference:
            ...
            interferer_per_frame = [[] for _ in range(self.radar_frames_per_timestep)]
            ...
            for i, frame in enumerate(interferer_per_frame):
                ...
                mask = np.isin(
                    element=obs_per_frame[i, :, 2],
                    test_elements=overlapping_ids_of_frame,
                )
                ...
                for j, radar in enumerate(affected_radars):
                    ...
                    # overwrite for the current of the observations that interfer
        ...
        for i in range(output_cells):
            ...
            output_obs[i, 0] = dist[i * combined_cells : (i + 1) * combined_cells].min()

            # set velocity to the velocity of the cell which is closest within the ones we combine
            closest_hit = dist[i * combined_cells : (i + 1) * combined_cells].argmin()
